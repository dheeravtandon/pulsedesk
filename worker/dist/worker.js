...
var require_news = __commonJS({
  "src/services/news.js"(exports, module) {
    ...
    async function fetchAll(limit = 15) {
      const results = await pool(FEEDS, 5, async (f) => parseFeed(await getText(f.url, { timeout: 11e3 }), f.source, f.region));
...
var require_weather = __commonJS({
  "src/services/weather.js"(exports, module) {
    ...
    var CODES = {
      ...
      55: ["Heavy drizzle", "\u{1F327}\uFE0F"],
      56: ["Freezing drizzle", "\u{1F327}\uFE0F"],
      57: ["Freezing drizzle", "\u{1F327}\uFE0F"],
      61: ["Light rain", "\u{1F326}\uFE0F"],
      63: ["Rain", "\u{1F327}\uFE0F"],
      65: ["Heavy rain", "\u26C8\uFE0F"],
      66: ["Freezing rain", "\u{1F327}\uFE0F"],
      67: ["Freezing rain", "\u{1F327}\uFE0F"],
      71: ["Light snow", "\u{1F328}\uFE0F"],
      73: ["Snow", "\u{1F328}\uFE0F"],
      75: ["Heavy snow", "\u2744\uFE0F"],
      77: ["Snow grains", "\u2744\uFE0F"],
      80: ["Rain showers", "\u{1F326}\uFE0F"],
      81: ["Rain showers", "\u{1F327}\uFE0F"],
      82: ["Violent showers", "\u26C8\uFE0F"],
      85: ["Snow showers", "\u{1F328}\uFE0F"],
      86: ["Snow showers", "\u{1F328}\uFE0F"],
      95: ["Thunderstorm", "\u26C8\uFE0F"],
      96: ["Storm + hail", "\u26C8\uFE0F"],
      99: ["Severe storm", "\u26C8\uFE0F"]
    };
    async function locate(manual) {
      ...
      const a = await settled(cachedJSON("geo:ipapi", "https://ipapi.co/json/", 6 * 60 * 60 * 1e3));
      if (a && a.latitude) {
        return { lat: a.latitude, lon: a.longitude, city: a.city, region: a.region, country: a.country_name, tz: a.timezone };
      }
      const b = await settled(cachedJSON("geo:ipwho", "https://ipwho.is/", 6 * 60 * 60 * 1e3));
      if (b && b.latitude) {
    ...
    async function today(manual) {
      ...
      const c = w.current || {};
      const d = w.daily || {};
      const [desc, icon] = CODES[c.weather_code] || ["\u2014", "\u{1F321}\uFE0F"];
      ...
      return {
        ...
        hourly: (w.hourly ? w.hourly.time : []).slice(start, start + 8).map((t, i) => ({
          ...
          icon: (CODES[w.hourly.weather_code[start + i]] || ["", "\u{1F321}\uFE0F"])[1]
        })),
        days: (d.time || []).slice(0, 3).map((t, i) => ({
          date: t,
          icon: (CODES[d.weather_code[i]] || ["", "\u{1F321}\uFE0F"])[1],
...
async function route(request, env, ctx) {
  ...
  switch (p) {
    ...
    case "/api/news":
      return cached(request, ctx, 300, () => import_news.default.fetchAll(15));
    case "/api/hyped":
    ...
    case "/api/health": {
      ...
      const checks = await Promise.all([
        ...
        probe("weather", () => import_weather.default.today({ lat: 19.1, lon: 72.9 }))
      ]);
