...
async function charter() {
  await writeDoc(`PD_Project_Charter_v${V}.docx`, [
    ...
    P('The five stocks drawing the most attention right now, ranked by a composite of unusual volume, the size of the day move, and how often the ticker appears in current headlines, with a "Show all" expansion into the full scanned list and a plain-English momentum read on each row. Satisfies FUN-001 to FUN-003, FUN-057 and FUN-061.'),
    H('3.2 Portfolio', HeadingLevel.HEADING_2),
    P('Holdings entry, live valuation, invested capital, unrealised and realised profit and loss, day profit and loss, allocation weights, idle cash and a net-worth trend, all expressed in a base currency of the user choosing. A holding can be entered either as a share quantity or as a money amount, with the other derived from the buy price. Satisfies FUN-004 to FUN-011, FUN-030 and FUN-054.'),
    ...
    P('A scrolling tape of Indian and US indices, commodities, USD/INR and Bitcoin; exchange session clocks stating plainly whether each is open now or closed, with a closes-in / opens-in countdown; and a breadth-derived market pulse score. Satisfies FUN-020 to FUN-022 and FUN-063.'),
    H('3.6 Weather', HeadingLevel.HEADING_2),
    P('Current conditions, feels-like temperature, humidity, wind, rain probability, UV, air quality and an eight-hour outlook for the user location, resolvable by IP or by manual coordinates, alongside a fixed strip of live conditions for six major financial-hub cities. Satisfies FUN-023, FUN-024 and FUN-062.'),
    ...
    H('3.13 Mutual funds', HeadingLevel.HEADING_2),
    P('A curated set of well-known Indian direct-growth mutual fund schemes spanning flexi cap, large/mid/small cap, index, hybrid and debt categories, each showing its current NAV and one-day, one-month and one-year change, filterable by category. Satisfies FUN-055 and FUN-056.'),
    H('3.14 Stock detail and related news', HeadingLevel.HEADING_2),
    P('Clicking any listed stock — in the hype radar, the popular list, a holding row or the all-scanned list — opens a price chart with Google-style 1D/5D/1M/6M/1Y/5Y range tabs and the most recent related headlines. The same related-news lookup surfaces inside the add-holding dialog once a company is picked, so recent context is visible before money is committed. Satisfies FUN-058 and FUN-059.'),
...
async function sdd() {
  await writeDoc(`PD_Design_Document_v${V}.docx`, [
    ...
    P('A purchase is either "right now", which uses the live quote, or an exact past date and time. For a past moment the service requests a window around that timestamp at the finest interval the provider still serves for that age — fifteen minutes inside two months, then hourly, then daily, then weekly — and picks the closest candle. Markets are shut most of the time, so the window widens step by step until a real trade exists, and the dialog says whether it found the exact moment or the nearest trading time. The dialog then shows the amount invested and, for past purchases, the gain or loss against the live price before anything is saved. The auto-filled price is always editable and is relabelled as the user\'s own figure once touched. A quantity/amount toggle lets the field hold either a share count or a money figure; in amount mode the quantity is derived as amount ÷ price before the preview and the save call. Satisfies FUN-041 to FUN-044 and FUN-054.'),
    H('4.11 Popular and steady', HeadingLevel.HEADING_2),
    P('A fixed list of thirty widely-held Indian and US large caps, each tagged with a sector, is fetched as a year of daily candles. Annualised volatility is computed from log returns and banded into very steady, steady, moves a lot and volatile; the list is ordered calmest first and shows the one-year return beside it, and can be narrowed by market or by sector using a dropdown populated from the data actually returned. This is deliberately the opposite selection to the hype radar, and the card states in plain text that it is not advice. Satisfies FUN-048, FUN-049 and FUN-060.'),
    H('4.12 Mutual funds', HeadingLevel.HEADING_2),
    P('Fourteen well-known Indian direct-growth schemes are pinned by their permanent AMFI scheme code rather than resolved by name search at run time, because mfapi.in\'s free-text search is capped and unranked and several of these funds have been renamed by their AMC since launch (for example "SBI Bluechip" is now "SBI Large Cap", "Kotak Emerging Equity" is now "Kotak Midcap"). Each scheme\'s full NAV history is fetched once, cached for six hours, and used to derive the latest NAV plus one-day, one-month and one-year change from the dated NAV series. Satisfies FUN-055, FUN-056 and SEC-016.'),
    H('4.13 Stock detail chart', HeadingLevel.HEADING_2),
    P('A single history function maps six range keys (1D, 5D, 1M, 6M, 1Y, 5Y) to a Yahoo range-and-interval pair and returns the full timestamped close series for that window, not just the sixty-point sparkline slice used elsewhere. The renderer draws this as an inline SVG area chart and refetches on each range-tab click. A shared matcher filters the already-fetched news wire by ticker or by company-name keyword — no extra network call — and the same matcher backs the recent-news strip in the add-holding dialog; the price chart is drawn from past prices only and is labelled as a pattern read, not a forecast. Satisfies FUN-058 and FUN-059.'),
